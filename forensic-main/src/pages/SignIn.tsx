...
export default function SignIn() {
    ...
    try {
      ...
      if (error.response) {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: status === 410 ? "Account does not exist with this email" : message,
        });
  ...
  useEffect(() => {
    ...
    if (!isSignIn || !formData.email) return;
  
    const registerUser = () => {
      socket.emit("registerUser", formData.email, (response) => {
        if (response.success) {
          console.log("✅ Registered user:", formData.email);
        } else {
            ...
            <>
              ...
              <div className="text-center text-sm">
                {resendTimer > 0 ? (
                  ...
                  <>
                    Didn’t receive the code?{" "}
                    <button onClick={resendOtp} className="font-medium text-primary hover:underline">
                      Resend OTP
                    </button>
                  </>
                )}
