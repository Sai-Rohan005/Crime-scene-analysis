...
export default function Case() {
  ...
  // Handle incoming calls
    ...
    useEffect(() => {
      socket.emit("register", decoded?.email);
  
      socket.on("incoming-call", ({ from, offer }) => {
        callerId.current = from;
    ...
    const offerRef = useRef<any>(null); // Store offer to use when user accepts
    ...
    const acceptCall = async () => {
      ...
      await peerRef.current.setRemoteDescription(new RTCSessionDescription(offerRef.current));
      const answer = await peerRef.current.createAnswer();
      await peerRef.current.setLocalDescription(answer);
  
      // setvideocallingon(true);
      socket.emit("acceptCall", { to: callerId.current, answer });
      navigate('')
  
      peerRef.current.onicecandidate = (e) => {
        if (e.candidate) {
          socket.emit("ice-candidate", { to: callerId.current, candidate: e.candidate });
        }
      };
    };
    ...
    const rejectCall = () => {
      if (callerId.current) {
        socket.emit("reject-call", { to: callerId.current });
      }
      setIsIncomingCall(false);
    };
